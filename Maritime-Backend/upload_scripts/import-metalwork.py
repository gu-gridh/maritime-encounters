...
for place, adm0n, adm1n, adm2n, adm3n, adm4n, provincen, parishn, x, y in df_clip[['place', 'ADM_0', 'ADM_1', 'ADM_2', 'ADM_3', 'ADM_4', 'province', 'parish', 'x', 'y']].drop_duplicates(['x','y','place']).values:
...
desc_df = pd.json_normalize(literal_eval(df_clip['certainContextDescriptors'][0]))
for category in desc_df.columns:
    ContextFindsCategories.objects.get_or_create(text=category.capitalize())

for row in df_clip.itertuples(index=False):
