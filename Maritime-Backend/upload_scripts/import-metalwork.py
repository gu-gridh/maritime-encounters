...
for place, adm0n, adm1n, adm2n, adm3n, adm4n, provincen, parishn, x, y in df[['place', 'ADM_0', 'ADM_1', 'ADM_2', 'ADM_3', 'ADM_4', 'province', 'parish', 'x', 'y']].drop_duplicates(['x', 'y', 'place']).values:
    ...
    adm2 = ADM2.objects.get(
        name=adm2n, ADM1__name=adm1n) if adm2n != None else None
    adm3 = ADM3.objects.get(
        name=adm3n, ADM2__name=adm2n) if adm3n != None else None
    adm4 = ADM4.objects.get(name=adm4n, ADM3__name=adm3n,
                            ADM3__ADM2__name=adm2n) if adm4n != None else None
    province = Province.objects.get(
        name=provincen) if provincen != None else None
...
for row in df.itertuples(index=False):
    ...
    db_object = Metalwork.objects.update_or_create(
        ...
        collection=MuseumCollection.objects.get_or_create(collection=row.museumCollection.strip().title())[
            0] if not pd.isnull(row.museumCollection) else None,
    ...
    if row.findsOverview != None:
        ...
        for finds in data.itertuples():
            ...
            object_desc = ObjectDescription.objects.get_or_create(
                subcategory=subcategory_text, category=category_text)[0]
